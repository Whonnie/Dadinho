...
def renderizar_tabuleiro_sincronizado():
    ...
    if estado_nuvem:
        ...
        if fim_clara and fim_julia:
            ...
            """, unsafe_allow_html=True)
            if st.button("🔄 JOGAR NOVAMENTE", use_container_width=True, type="primary", key="btn_jogar_novamente_fim"):
        ...
        with Mesa:
            ...
            with col_debajo:
                ...
                if st.button(cor_botao_debajo, use_container_width=True, key="btn_debajo_toggle"):
                    alternar_por_debajo(estado_nuvem)
                    st.rerun()
                    
            with col_action:
                if st.button("🎲 ROLAR DADOS", type="primary", use_container_width=True, key="btn_rolar_dados_acao"):
                    rolar_dados_web(estado_nuvem)
                    st.rerun()
                    
            st.markdown("<br><hr style='border-color: rgba(255,255,255,0.1);'>", unsafe_allow_html=True)
            
            if st.button("🔄 ZERAR PARTIDA ATUAL", use_container_width=True, key="btn_zerar_partida_esquerda"):
                reiniciar_partida_completa()
        ...
        with Placar:
            ...
            # RENDERIZAÇÃO EXCLUSIVA NO TOPO DO RETÂNGULO ESCURO
            st.markdown(f"""
